...
class DonRepository extends ServiceEntityRepository {
    ...
    public function rechercheDon($criteria = [], DateTime $dateDebut = null, DateTime $dateFin = null, $limit = NULL) {
        try {
...
//            $qb->addOrderBy('enf.date2', 'DESC');
            ...
            if ($dateDebut && $dateFin) {
                $qb
                        ->andWhere('enf.datedon BETWEEN :dateDebut AND :dateFin')
                        ->setParameter('dateDebut', $dateDebut)
                        ->setParameter('dateFin', $dateFin);
            }
            if (array_key_exists('nature', $criteria)) {

                $qb->andWhere("enf.nature = :nature");
                $qb->setParameter('nature', $criteria['nature']);
                unset($criteria['nature']);
            }
            if ($limit) {
    ...
    public function rechercheDonn($criteria = [], DateTime $dateDebut = null, DateTime $dateFin = null, $limit = NULL) {
        try {
            $qb = $this->createQueryBuilder('enf');
