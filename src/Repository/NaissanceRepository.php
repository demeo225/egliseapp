...
class NaissanceRepository extends ServiceEntityRepository {
    ...
    public function rechercheNaissance($criteria = [], DateTime $dateDebut = null, DateTime $dateFin = null, $limit = NULL) {
        try {
...
//            $qb->addOrderBy('enf.date2', 'DESC');
            ...
            if ($dateDebut && $dateFin) {
                $qb
                        ->andWhere('enf.datepresentation BETWEEN :dateDebut AND :dateFin')
                        ->setParameter('dateDebut', $dateDebut)
                        ->setParameter('dateFin', $dateFin);
            }
            if (array_key_exists('sexenaiss', $criteria)) {

                $qb->andWhere("enf.sexenaiss = :sexenaiss");
                $qb->setParameter('sexenaiss', $criteria['sexenaiss']);
                unset($criteria['sexenaiss']);
            }
            if ($limit) {
           ...
           public function rechercheNaissancen($criteria = [], DateTime $dateDebut = null, DateTime $dateFin = null, $limit = NULL) {
        try {
            $qb = $this->createQueryBuilder('enf');
